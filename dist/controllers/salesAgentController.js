...
var sales_agent_1 = require("./../models/sales-agent");
var SalesAgentController = /** @class */ (function () {
    ...
    SalesAgentController.prototype.add = function (req, res) {
        if (req.user.role == "admin") {
            var salesAgent = new sales_agent_1.SalesAgent(req.body);
    ...
    SalesAgentController.prototype.getAll = function (req, res) {
        sales_agent_1.SalesAgent.find({}, function (err, salesAgents) {
            if (err) {
                res.send(err);
            }
    ...
    SalesAgentController.prototype.getByKey = function (req, res) {
        sales_agent_1.SalesAgent.findOne({ key: req.params.id }, function (err, salesAgent) {
            if (err) {
                res.send(err);
            }
            res.json(salesAgent);
    ...
    SalesAgentController.prototype.update = function (req, res) {
        if (req.user.role == "admin") {
            sales_agent_1.SalesAgent.findOneAndUpdate({ key: req.params.id }, req.body, { new: true }, function (err, salesAgent) {
                if (err) {
                    res.send(err);
                }
                res.json(salesAgent);
    ...
    SalesAgentController.prototype.delete = function (req, res) {
        if (req.user.role == "admin") {
            sales_agent_1.SalesAgent.remove({ key: req.params.id }, function (err, salesAgent) {
                if (err) {
                    res.send(err);
                }
